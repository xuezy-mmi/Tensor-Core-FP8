...
void BmFN(int M, int K, int N, int kernel, bool func, bool record){
    ...
    checkCuda(cudaMalloc(&d_Output_Value, (M*K) * sizeof(CType)));
    // copy value into gpu
    checkCuda(cudaMemcpy(d_A_Value, h_A_Value, (M*K) * sizeof(AType), cudaMemcpyHostToDevice));
    checkCuda(cudaMemcpy(d_B_Value, h_B_Value, (M*K) * sizeof(BType), cudaMemcpyHostToDevice));
    ...
    if(func){//verify
        ...
        for(int i = 0; i < M*N; i++){
            float temp_error = abs((float)Output_Value_base[i] - (float)h_Output_Value[i]);
            if(max_error > 0.5){
                num_errors += 1;
            }
...
void usage(void){
    printf("!!!!!!!\n");
    printf("Input Help!\n");
    printf("Input Format is  ./gemm_main [dimM] [dimK] [dimN] [kernel] [verify] [reocrd]\n");
...
int main(int argc, char **argv){
    ...
    // // Run the benchmark
    // else{
    const int dimM = std::atoi(argv[1]);//A_rows_num
    const int dimK = std::atoi(argv[2]);//A_cols_num // B_rows_num
    const int dimN = std::atoi(argv[3]);//B_cols_num
    ...
    if(kernel == 1){
        ...
        BmFN<__nv_fp8_e4m3, __nv_fp8_e4m3, half>(dimM, dimK, dimN, kernel, verify, record);
    }
    else if(kernel == 2){
        ...
        BmFN<__nv_fp8_e5m2, __nv_fp8_e5m2, half>(dimM, dimK, dimN, kernel, verify, record);
    }
    else if(kernel == 4){
        ...
        BmFN<__nv_fp8_e4m3, __nv_fp8_e5m2, half>(dimM, dimK, dimN, kernel, verify, record);
    }
    else if(kernel == 6){
        ...
        BmFN<__nv_fp8_e5m2, __nv_fp8_e4m3, half>(dimM, dimK, dimN, kernel, verify, record);
    }
    else if(kernel == 8){
    ...
    //     if(vec_length == 8){
    //         benchmark = "../benchmark/dlmc/";
    //     }
    //     else if(vec_length == 16){
    //         benchmark = "../benchmark/dlmc-v16/";
    //     }
    //     else{
    //         benchmark = "../benchmark/dlmc/";
    //     }
    ...
    //         if (mixed) BmFN<half, half, int, float, int>(dimM, dimK, dimN, kernel, verify, record);
    //         else BmFN<half, float, int, float, int>(dimM, dimK, dimN, kernel, verify, record);
    //         file_count++;
    //     }
    //     infile.close();
    //     printf("matrix num:%d\n", file_count);
